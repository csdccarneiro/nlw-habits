        ...
        <View className="flex-1 bg-background px-8 pt-16">
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
                ...
                <TextInput 
                    ...
                    value={title}
                />
                <Text className="mt-4 mb-3 text-white font-semibold text-base">Qual sua recorrência?</Text>
                {
                    availableWeekDays.map((weekDay, index) => (
                        <CheckBox title={weekDay} key={index} onPress={() => handleToogleWeekDays(index)} checked={weekDays.includes(index)} />
                    ))
                }
                
                <TouchableOpacity className="w-full h-14 flex-row items-center justify-center bg-green-600 rounded-md mt-6" 
                    activeOpacity={0.7} onPress={handleCreateNewHabit}>
